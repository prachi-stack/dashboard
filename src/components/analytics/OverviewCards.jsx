	...
	return (
		<div className='grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4 mb-8'>
			{overviewData.map((item, index) => (
				<motion.div
					key={item.name}
					className='bg-brown-600 bg-opacity-50 backdrop-filter backdrop-blur-lg shadow-lg
            rounded-xl p-6 border border-brown-500
          '
				...
				>
					<div className='flex items-center justify-between'>
						<div>
							...
							<p className='mt-1 text-xl font-semibold text-brown-100'>{item.value}</p>
						</div>
            ...
            `}
					>
						...
						<span className='ml-2 text-sm text-brown-200'>vs last period</span>
					</div>
				</motion.div>
			))}
		</div>
